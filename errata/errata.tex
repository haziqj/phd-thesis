...
\begin{enumerate}
  ...
  \citet{jaynes1957a,jaynes1957b,jaynes2003probability} argues that in the absence of any prior knowledge, a probability distribution that maximises information entropy (as per definition \hl{XX}) should be advocated.
  ...
  In Chapter 3, we show that the entropy maximising prior distribution for the regression function is Gaussian with covariance kernel proportional to its Fisher \underline{i}nformation (hence the name \underline{I}-prior).
...
%  Having said that, the logit link is certainly possible, but estimation has to be done differently e.g. using Laplace approximation (\hl{Rasmussen \& Williams}), a local variational bound (\hl{Bishop 2006}), or some other method.
  ...
  A suggestion of using INLA was received.
  ...
  \textit{Amendments to thesis}: On Page 201, the following was added:
...
\end{displayquote} 
  ...
  On page 207, a reference was made to ``model-averaged'' coefficients.
  ...
  To clarify, in our data applications in Sections 6.7.1 and 6.7.2 report the model-averaged regression coefficients, but no attempt of interpreting these coefficients were made.
  The data example in Section 6.7.3 highlights predictive accuracy of the choice of I-priors in Bayesian variable selection, in which predictive RMSE was made using model-averaged regression coefficients.
  ...
  \textit{Amendments to thesis}: Added remark on page 207 to clarify that the use of model-averaged regression coefficients for explanatory inference is not meaningful.
