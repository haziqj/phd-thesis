...
\]
where $k$ is the index of the parameter in the set $\mathcal E$. The \pkg{iprior} package employs Wald tests of significance based off of these standard errors for the intercept and scale parameters in the \code{summary} of an \code{ipriorMod} object.
...
In the \pkg{iprior} package and throughout this paper, we use centred kernels, where the centering is done with respect to the empirical distribution of the relevant covariate; if the covariate observations are $x_1,\ldots,x_n\in\cal X$, then the centred kernel $\tilde h$ is defined as
