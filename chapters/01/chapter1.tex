...
%\cref{chapter4,chapter5,chapter6} 
...
\tikzstyle{container} = [draw, rectangle, inner sep=0.4cm, fill=black!10]
...
\begin{tikzpicture}
  ...
  \node [block, below=1.2cm of ch1] (ch2) {\small \textbf{Chapter 2}\\Vector space of functions};
  \node [block, right of=ch2] (ch3) {\small \textbf{Chapter 3}\\The I-prior};
  \node[draw=none,above=0.38cm of ch3] (title) {\small\textbf{Main chapters}}; 
  \node [block, below=1cm of ch3] (ch4) {\small \textbf{Chapter 4}\\Regression with I-priors};
  \node [block, right of=ch4] (ch5) {\small \textbf{Chapter 5}\\Categorical responses};     
  \node [block, text width = 2.5cm, left of=ch4]  (ch6) {\small \textbf{Chapter 6}\\Bayesian vari-\\able selection};   
  \node [block, right=1.3cm of ch5]  (ch7) {\small \textbf{Chapter 7}\\Summary};  
\node [block, above=1.3cm of ch7]  (apx) {\scriptsize \textbf{Appendices A--I}\\ References, derivations and proofs};              
  \node [block, below=1.3cm of ch6]  (s1) {\small \textbf{Supp. 1}\\Estimation concepts};
  \node [block, below=1.3cm of ch4]  (s2) {\small \textbf{Supp. 2}\\EM algorithm};   
  \node [block, below=1.3cm of ch5]  (s4) {\small \textbf{Supp. 4}\\Hamiltonian Monte Carlo};         
  \node [block, right of=s4]  (s3) {\small \textbf{Supp. 3}\\Variational inference};
  \begin{scope}[on background layer]
    \node [container,fit=(ch2) (ch3) (ch4) (ch5) (ch6) ] (container) {};
  \end{scope}
    
  \path (ch1) edge [-latex, thick, shorten >=0.365cm] (ch2)
        (ch2) edge [-latex, thick] (ch3)
        (ch3) edge [-latex, thick] (ch4)
        (ch4) edge [-latex, thick] (ch5)
        (ch4) edge [-latex, thick] (ch6)
        (ch5) edge [-latex, thick, shorten <=0.4cm] (ch7)
        (s1)  edge [-latex, thick, dashed] (ch6)
        ...
        (apx)  edge [-latex, thick, dashed, shorten >=4cm] ($(ch3.east) + (0,0.45cm)$);

\end{tikzpicture}

  \caption[Schematic representation of the organisation of the chapters and supplementary chapters of this thesis]{Schematic representation of the organisation of the chapters and supplementary chapters of this thesis. Solid lines indicate requisite relevances, while dashed lines indicate supporting and supplementary relevances.}
\end{figure}
