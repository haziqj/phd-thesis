In the introductory chapter (\cref{sec:introregmod}), we described several interesting regression models.
...
\hltodo[Rethink]{If the covariates are not scaled similarly, then the values of $f$ are incoherent---if $x_1$  measures weight in kilograms and $x_2$ height in centimetres, what measurement does $\beta_1x_1 + \beta_2x_2$ represent?}
...
\begin{remark}\hltodo[Anything wrong with standardising?]{}
  ...
  We feel this is a rather ad-hoc approach---how can one be sure that the variables are centred and scaled appropriately?
...
\begin{itemize}
  \item $f(\bx_i^{(j)}, j) = \alpha + f_1(\bx_i^{(j)}) + f_2(j)$ (random intercept model);  %\cF_0 \oplus \lambda_1\cF_1 \oplus \lambda_2\cF_2
...
\begin{align}\label{eq:longitudinalanova}
  f(x_i,t) = \alpha + f_1(i) + f_2(x_i) + f_3(t) + f_{13}(i,t) + f_{23}(x_i,t) + f_{123}(i,x_i,t),
\end{align}
where $\alpha$ is an overall constant, $f_k\in\cF_k$, $k=1,2,3$,  $f_{j3}\in\cF_j\otimes\cF_3$, $j=1,2$, and $f_{123}\in\cF_1\otimes\cF_2\otimes\cF_3$.
...
\subsection{Classification}\label{sec:naiveclass}
\input{05a-naive}
