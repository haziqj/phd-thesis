...
\begin{enumerate}[label=A\arabic*,ref=A\arabic*]
  ...
  \item \textbf{Iid errors}. Assume identical and independent errors, i.e. $\bPsi = \psi\bI_n$. \label{ass:A3} 
\end{enumerate}
...
Direct optimisation is typically done using conjugate gradients with a Cholesky decomposition on the covariance kernel to maintain stability, but we opt for an eigendecomposition of the kernel matrix $\bH_{\eta} = \bV \, \text{diag}(u_1,\dots,u_n) \, \bV^\top$ instead.
...
Standard calculations \colp{\cref{apx:fishermultinormal}} show that under the marginal distribution $\tilde\by \sim \N_n\big(\bzero, \bSigma_\theta \big)$, the $(i,j)$'th coordinate of $\bU$ is 
\begin{equation}
  u_{ij} = 
  \half \tr\left(
...
Log-likelihood    & -339.7              & -339.7       & -341.1                  \\
