...
%  \begin{cases}
%    \tilde\bV_u[i,j] + \wtilde_{\bigcdot i}^\top  \wtilde_{\bigcdot j} &\text{(full I-probit)} \\
%        \delta_{ij}\tilde\bV_{u_j} + \wtilde_{\bigcdot i}^\top  \wtilde_{\bigcdot j} &\text{(independent I-probit)}
%  \end{cases}
%  &=: \diag(\tilde\bV_{u_1},\dots,\tilde\bV_{u_m}).
\end{align*}

In the case of the I-probit model, we use a gamma prior on each of the precisions in the diagonal entries of $\bPsi=\diag(\psi_1,\dots,\psi_m)$.
