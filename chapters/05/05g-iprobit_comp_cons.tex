...
\begin{remark}
  ...
  This is similar to fixing the latent variables of the reference category to zero, and thus, as discussed previously in \hltodo{section}, full identification is achieved by fixing one more element of the covariance matrix.
\end{remark}
...
\begin{equation*}
  ...
  \end{cases}
\end{equation*}
%Thus, the original integral in \cref{eq:pij} involving an $m$-variate normal density is turned into a product of $m-1$ univariate normal cdfs, which can be computed fairly efficiently in modern computer systems.
The GHK algorithm entails making draws from truncated standard normal distributions (for instance, using an inverse transform method detailed in \hltodo{appendix}):
\begin{itemize}
...
%Prediction of a single new data point is $O(n^2m)$ in time, because there are essentially $m$ I-prior posterior regression functions, and each take $O(n^2)$ to evaluate.
...
\end{align*}
Setting this to zero and solving for $\bPsi$ yields the M-step update equation for $\bPsi$\hltodo[Actually, I don't know how to solve this?]{}. 
...
As per the derivations in \hltodo{section}, the numerator of this expression is equal to $\tr(\tilde\bV_{w_j} + \tilde\bw_{\bigcdot j}\tilde\bw_{\bigcdot j}^\top) = \tr \tilde\bW_{jj}$.
...
Although this strategy can be used when $\bPsi$ is diagonal, in \cref{thm:contruncn}, we show that the form for the second moments  involve integration of standard normal cdfs and pdfs, much like in formula for the first moments.
