...
\begin{example}[Fisher information for linear regression]
  As before, suppose model \eqref{eq:model1} subject to \eqref{eq:model1ass} and $f\in\cF$, an RKHS.
...
\begin{corollary}[Fisher information between two linear functionals of the regression function]\label{thm:fisherreglinfunc}
	For our regression model as defined in \eqref{eq:model1} subject to \eqref{eq:model1ass} and $f$ belonging to a RKHS $\mathcal F$ with kernel $h$, the Fisher information at two points $f(x)$ and $f(x')$
...
An inspection of the formula in Corollary \ref{thm:fisherreglinfunc} reveals the fact that the Fisher information for $f(x)$, $\cI \big(f(x),f(x) \big)$, is positive if and only if $h(x,x_i)\neq 0$ for at least one $i \in \{1,\dots,n \}$.
