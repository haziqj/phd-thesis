...
\begin{definition}[Isometric isomorphism]
  Two Hilbert spaces $\cF$ and $\cG$ are said to be \emph{isometrically isomorphic}, symbolised $\cF \cong \cG$, if there is a linear bijective map $U:\cF\to\cG$ which preserves the inner product, i.e. for any $f,f'\in\cF$, \vspace{-0.2em}
  %
  \[
...
$A_g$ is obviously linear (bilinearity of inner products), and using the polarisation identity,
\begin{align*}
  2\ip{Ug,Ug'}_{\cF^*} 
  &= \norm{U(g)}_{\cF^*}^2 + \norm{U(g')}_{\cF^*}^2 - \norm{U(g - g')}_{\cF^*}^2 \\
  &= \norm{g}_{\cF}^2 + \norm{g'}_{\cF}^2 - \norm{g-g'}_{\cF}^2 \\
...
That is, for each pair of elements $(x_1,x_2) \in \cH_1\times\cH_2$, we define the (rank one) operator $A_{x_1,x_2}:\cH_1\to\cH_2$ in the following way:
\begin{align*}
...
\begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
  ...
  %
  \[
    x_1 \otimes x_2 \in \cH_1 \otimes \cH_2. \vspace{-0.6em}
  \]
  %
  \item \textbf{Operator}.
  \vspace{-1.2em}
  ...
  \begin{align*}
    ...
    y_1 &\mapsto \ip{x_1,y_1}_{\cH_1}x_2 \vspace{-1.2em}
  \end{align*}  
  %
