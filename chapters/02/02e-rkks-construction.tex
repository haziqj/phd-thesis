...
%We shall jump straight into the multiway ANOVA analogue for functional decomposition, and to that end, 
...
This gives us a suggestion as to what the ANOVA decomposition $f \in \cF$ should look like in general.
...
%$\cF_0$ is a vector space of constant functions, and 
...
\begin{align}
  ...
  &= \cF_{0}^{\otimes p} 
  \ \oplus \
  \bigoplus_{j=1}^p %\! \mathop{\vphantom\oplus}^\bot 
  \Big( \cF_0^{\otimes (p-1)} \otimes \bar\cF_j \Big) 
  \ \oplus \
  \mathop{\bigoplus_{j,k=1}^p}_{j<k} %\!\! \mathop{\vphantom\oplus}^\bot  
