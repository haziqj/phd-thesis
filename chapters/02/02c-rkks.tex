...
\begin{definition}[Negative and indefinite inner products]
  Let $\ip{\cdot,\cdot}_\cF$ be an inner product of a vector space $\cF$, as per Definition \ref{def:innerprod}.
...
One might wonder whether the uniqueness theorem (Theorem \ref{thm:rkhsunique}) holds for RKKS.
...
The motivation for the use of Kreĭn spaces will become clear when constructing function spaces out of (scaled) building block RKHS later in Section \ref{sec:constructrkks}.
