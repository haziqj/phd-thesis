...
Besides invariance, the ML estimate comes with the attractive limiting property $\surd{n}(\theta_{\text{ML}} - \theta_{\text{true}}) \xrightarrow{\text{dist.}} \N(0,\cI)$ \citep{casella2002statistical} as sample size $n\to\infty$, where $\cI$ is the Fisher information.
In words, the ML estimate is consistent, efficient, and asymptotically normal.
