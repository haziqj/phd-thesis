...
Bayesian model selection is probabilistic in nature: a priori, one assigns probabilities over the model space, and then after observing the data, posterior model probabilities are used to discern which of the models was likeliest to have been behind the data generative process of the observed responses.
...
It would be useful to be able to index each of these $2^p$ possible models somehow, and we achieve this by the use of indicator variables.
...
Following \citet{Kuo1998}, the linear model in \cref{eq:linmod} is expanded to include the indicator variables $\gamma_j$ to form
...
Note that as $c,d \to 0$ in the inverse gamma distribution we get the Jeffrey's prior for scale parameters.
...
The Gibbs conditional densities are straightforward to obtain on account of model conjugacy (details of derivation in \cref{apx:gibbsbvs}).
...
\end{align*}
Therefore it can be seen that the decision to include or exclude the $j$'th variable from the model relates a hypothesis test using the Bayes factor rule, and this decision is embedded in the conditional posterior probabilities $\tilde \pi_j$.
...
Here, $c_\sigma$, $d_\sigma$, $c_\kappa$ and $d_\kappa$ are the shape and scale hyperparameters of the inverse gamma priors on $\sigma^2$ and $\kappa$ respectively.
...
\hltodo[Not sure how to word this.]{As a result, information summarised through the posterior inclusion probabilities are  reliable}.
...
\begin{figure}[hbt]
  \centering
  \includegraphics[width=\textwidth]{figure/06-sens_analysis}
  \caption[Sensitivity analysis of hyperprior choice on number of false choices]{Average number of false choices (false inclusions or false exlusions) for the five different scenarios (SNR varied between 90\%, 75\%, 50\%, 25\% and 10\%) with different hyperprior setting for $\gamma_j\sim(\pi_j)$.}
  \label{fig:simres2}
\end{figure}
\vspace{-1em}
...
The sample correlations of interest are shown in Figure \ref{fig:aerobic-cor}.
...
The disagreement with the principle of model selection is that one should build models by thoughtfully choosing variables which are expected to influence the response by appealing to substantive theory, and not by virtue of optimising some model selection criterion.
...
\begin{enumerate}
  ...
  Variational inference with spike-and-slab priors on regression coefficients was studied by \citep{ormerod2017variational}.
