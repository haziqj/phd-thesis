...
Minor differences do exist, with the most noticeable one being that the slopes in the I-prior model are categorically estimated as zero.
Even so, the conclusions from both models are similar.
We also note that the sign of the estimated correlation $\rho_{01}$ is the same in both models.
...
\subsection{Regression with a functional covariate}\label{sec:regfunccov}
