...
knitr::read_chunk("05-02-cardiac.R")
knitr::read_chunk("05-04-smoking.R")
knitr::read_chunk("05-05-vowel.R")
knitr::read_chunk("05-06-btb.R")
@
...
@
<<cardiac.mod.full.plot, echo = FALSE, fig.height = 3.9 * 0.9, fig.width = 6.5 * 0.9, out.width = "0.785\\linewidth", fig.cap = "Plot of variational lower bound over time (top), and plot of training error rate and Brier scores over time (bottom).", fig.scap = "Plot of variational lower bound, training error rate and Brier scores against time and iterations", fig.show = "hold", fig.pos = "htb">>=
@
...
\begin{table}[htb]
  \caption[Mean out-of-sample misclassification rates for the cardiac arrhythmia data set]{Mean out-of-sample misclassification rates and standard errors in parantheses for 100 runs of various training ($s$) and test ($451-s$) sizes for the cardiac arrhythmia binary classification task.}
  \label{tab:cardiac}
  \centering
...
@
<<plot.data.smoke, echo = FALSE, fig.height = 4, message = FALSE, fig.cap = "Comparative box-plots of the distribution of patients who successfully quit smoking and those who remained smokers, in the two treatment groups. It is evident that there are more successful patients quitting smoking in the treatment group than in the control group. The raw odds ratio of quitting smoking (treatment vs. control) is 1.66.", fig.pos = "htb">>=
@
A summary of the data is displayed by the box-plot in \cref{fig:plot.data.smoke}.
On the whole, there are a total of 5,908
% \Sexpr{study.size[length(study.size)]}
patients, and they are distributed roughly equally among the control and treatment groups (\Sexpr{iprior::dec_plac(dist.mean[2], 1)}\% and \Sexpr{iprior::dec_plac(dist.mean[1], 1)}\% respectively, on average).
...
The overall log odds ratio is represented by $\beta_1$, and this is estimated as $\Sexpr{iprior::dec_plac(smoke.re.res$logodds[28])} \approx \log \Sexpr{iprior::dec_plac(exp(smoke.re.res$logodds[28]))}$.
...
This has been calculated as $\Sexpr{iprior::dec_plac(smoke.ip.res$logodds[28])} \approx \log \Sexpr{iprior::dec_plac(exp(smoke.ip.res$logodds[28]))}$, slightly lower than both the raw log odds ratio and the log odds ratio estimated by the logistic random-effects model.
...
<<vowel.confusion.matrix, echo = FALSE, fig.show = "hold", fig.height = 4.5, fig.width = 5, out.width = "0.485\\linewidth", fig.cap = "Confusion matrices for the vowel classification problem in which predicted values were obtained from the I-probit models. The maximum value for any cell is 42 (seven speakers delivered six frames of speech per vowel). Blank cells indicate nil values.", fig.subcap=c("Canonical kernel", "fBm-0.5 kernel", "SE kernel"), fig.pos = "htb">>=
@
...
Additionally, Gaussian process classification (SE kernel) using the \pkg{kernlab} package \citep{zeileis2004kernlab} in \proglang{R} was used.
The I-probit model using the fBm-0.5 and SE kernel offers two of the best out-of-sample classification error rates (40\% and 34\% respectively) of all the methods compared.
The linear I-probit model is seen to be comparable to logistic regression, linear and quadratic discrimant analysis, and also decision trees, but it provides significant improvement over multiple linear regression.
...
\index{bovine tuberculosis (BTB) data set}
\index{ANOVA!kernel/RKKS}
\index{fBm kernel/RKHS}
\index{Pearson kernel/RKHS}
...
<<plot.cow, echo = FALSE, fig.height = 4, fig.width = 8, out.width = "\\linewidth", fig.cap = "Distribution of the different types (Spoligotypes) of bovine tubercolosis affecting herds in Cornwall over the period 1989 to 2002.", fig.pos = "htb">>=
@
...
<<plot.cornwall, echo = FALSE, fig.height = 7.2, fig.width = 8, out.width = "\\linewidth", fig.cap = "Spatial distribution of all cases over the 14 years.", fig.pos = "htb">>=
@
...
<<plot.btb, echo = FALSE, fig.height = 7.5, fig.width = 8, out.width = "\\linewidth", fig.cap = "Predicted probability surfaces for BTB contraction in Cornwall for the four largest spoligotypes of the bacterium \\emph{Mycobacterium bovis} over the entire time period using model $M_1$.", fig.scap = "Predicted probability surfaces of BTB contraction using model $M_1$", fig.pos = "p", dev = "png", dpi = 300, cache = TRUE>>=
@
<<plot.temporal.btb, echo = FALSE, warning = FALSE, fig.height = 7.5, fig.width = 8, out.width = "\\linewidth", fig.cap = "Predicted probability surfaces for BTB contraction in Cornwall for the four largest spoligotypes of the bacterium \\emph{Mycobacterium bovis} over four different time periods using model $M_3$.", fig.scap = "Predicted probability surfaces of BTB contraction using model $M_3$", fig.pos = "p", dev = "png", dpi = 300, cache = TRUE>>=
@
...
The occurrences of spoligotype 9 in the south-west, which is most commonly seen in the east of Cornwall, is not deemed to be significant by the model.
The other two spoligotypes are also relatively unchanged across the years.
