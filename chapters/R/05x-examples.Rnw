...
All of these results are tabulated in \cref{tab:cardiac}.
...
patients, and they are distributed roughly equally among the control and treatment groups (\Sexpr{iprior::dec_plac(dist.mean[2])}\% and \Sexpr{iprior::dec_plac(dist.mean[1])}\% respectively, on average).
...
Other work includes \citet{diggle2013spatial}, who used a fully Bayesian approach for spatio-temporal multivariate log-Gaussian Cox processes, and implemented in the \proglang{R} package \pkg{lgcp} \citep{taylor2013lgcp}.
