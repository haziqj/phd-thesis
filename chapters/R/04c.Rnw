...
knitr::read_chunk("04-02-longitudinal.R")
@
...
<<ipriorKernels, include = FALSE, cache = TRUE, cache.lazy = FALSE>>=
@
<<model_fitting, echo = c(1,2), cache = TRUE, cache.lazy = FALSE>>=
@
...
\midrule
1     & \code{time}                   & \Sexpr{decPlac(mod1.fit$log.lik, 2)} & \Sexpr{decPlac(mod1.fit$sigma, 2)} & \Sexpr{length(mod1.fit$lambda)} \\
2     & \code{id * time}                & \Sexpr{decPlac(mod2.fit$log.lik, 2)} & \Sexpr{decPlac(mod2.fit$sigma, 2)} & \Sexpr{length(mod2.fit$lambda)} \\
3     & \code{group * time}             & \Sexpr{decPlac(mod3.fit$log.lik, 2)} & \Sexpr{decPlac(mod3.fit$sigma, 2)} & \Sexpr{length(mod3.fit$lambda)} \\
4     & \code{id * time + group * time}   & \Sexpr{decPlac(mod4.fit$log.lik, 2)} & \Sexpr{decPlac(mod4.fit$sigma, 2)} & \Sexpr{length(mod4.fit$lambda)} \\
5     & \code{id * group * time}          & \Sexpr{decPlac(mod5.fit$log.lik, 2)} & \Sexpr{decPlac(mod5.fit$sigma, 2)} & \Sexpr{length(mod5.fit$lambda)} \\
\bottomrule
\end{tabular}
...
The results of the model fit are summarised in Table \ref{tab:cowresults}. We can test for a treatment effect by testing Model 4 against the alternative that Model 2 is true. The log-likelihood ratio test statistic is $D = -2(\Sexpr{decPlac(mod2.fit$log.lik, 2)} - (\Sexpr{decPlac(mod4.fit$log.lik, 2)})) = \Sexpr{decPlac(-2*(mod2.fit$log.lik - mod4.fit$log.lik))}$ which has an asymptotic chi-squared distribution with $\Sexpr{length(mod4.fit$lambda)} - \Sexpr{length(mod2.fit$lambda)} = \Sexpr{length(mod4.fit$lambda) - length(mod2.fit$lambda)}$ degree of freedom. The $p$-value for this likelihood ratio test is less than $10^{-6}$, so we conclude that Model 4 is significantly better than Model 2.

<<cow_plot, echo = FALSE, fig.height = 4, fig.width = 7, out.width = "\\linewidth", fig.cap = "A plot of the I-prior fitted regression curves from Model 5. In this model, growth curves differ among cows and by treatment effect (with an interaction between cows and treatment effect), thus producing these 60 individual lines, one for each cow.", fig.pos = "h">>=
@
